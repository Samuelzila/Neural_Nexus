...
for i in range(16):
    ...
    main_frame.grid_columnconfigure(i, weight=1)  # Chaque colonne a un poids égal
...
result_frame.grid(row=0, column=8,rowspan=8 ,columnspan=8, padx=10, pady=10)  # Ligne 39
...
class DrawingApp(ctk.CTkFrame):
    ...
    def predict(self):
        ...
        img_resized = Image.fromarray(img_array).resize((236, 172)) #les dimensions sont durs à bien estimer si jamais y'a une a,mélioration à faire je suis preneur

        # Utiliser CTkImage au lieu de ImageTk.PhotoImage
        img_tk = CTkImage(light_image=img_resized, size=(236, 172)) #les dimensions sont durs à bien estimer si jamais y'a une a,mélioration à faire je suis preneur
